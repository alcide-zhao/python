...
layer_e = 0
for iyear in range(1961,2007):
	print iyear
	file= input_path + 'APHRO_MA_TAVE_050deg_V1204R1.'+str(iyear)+'.nc'
	nc_fid = nc4.Dataset(file,mode='r')
	lat = nc_fid.variables['latitude'][:]
	lon = nc_fid.variables['longitude'][:]
	temp = nc_fid.variables['tave'][:]
	units = nc_fid.variables['tave'].units
	long_name = nc_fid.variables['tave'].long_name
	missing_value = nc_fid.variables['tave'].missing_value
	...
	precip_series[layer_b:layer_e,:,:] = temp

file_name_out = input_path+'APHRO_MA_050deg_V1101_1951_2006.nc'
...
f.description = 'APHRO_MA V1101 daily temperature with 0.50deg grids 1961_2006'
