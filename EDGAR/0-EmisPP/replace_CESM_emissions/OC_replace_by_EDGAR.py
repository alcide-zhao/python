...
####OC_surf
# OC_surf_file=path_rcp85+'RCP85_mam3_oc_surf_2000-2100_c20110913.nc'
OC_surf_file=path_rcp85+'RCP85_mam3_oc_surf_2000-2100_c20110913.nc'
nc_surf = nc4.Dataset(OC_surf_file,mode='r+')
for imonth in months:
	edgar_file = path_EDGAR+fn_op+'_OC_'+str(year)+'_'+str(imonth+1)+'_NUM_MOL.0.1x0.1.nc'  #filename month starts from 1
	edgar_fid=nc4.Dataset(edgar_file,mode='r')
	OC_a1_surf_mol = edgar_fid.variables['POM_a1_srf_mol'][:]
	lat = edgar_fid.variables['lat'][:] 
	lon = edgar_fid.variables['lon'][:] 
	OC_a1_surf_mol=regrid_edgar2cesm(lon,lat,OC_a1_surf_mol,CESM_gc_area)
	nc_surf.variables['emiss_awb'][layer_b+imonth,:,:]=OC_a1_surf_mol
...
for imonth in months:
	edgar_file = path_EDGAR+fn_op+'_OC_'+str(year)+'_'+str(imonth+1)+'_NUM_MOL.0.1x0.1.nc'  #filename month starts from 1
	edgar_fid=nc4.Dataset(edgar_file,mode='r')
	OC_a1_srf_num= edgar_fid.variables['POM_a1_srf_num'][:] 
	lon = edgar_fid.variables['lon'][:]
	lat = edgar_fid.variables['lat'][:] 
	OC_a1_srf_num=regrid_edgar2cesm(lon,lat,OC_a1_srf_num,CESM_gc_area)
	num_a1_surf.variables['OC_emiss_awb'][layer_b+imonth,:,:]=OC_a1_srf_num
	num_a1_surf.variables['OC_emiss_wst'][layer_b+imonth,:,:]=0
	num_a1_surf.variables['OC_emiss_shp'][layer_b+imonth,:,:]=0
	num_a1_surf.variables['OC_emiss_dom'][layer_b+imonth,:,:]=0
	num_a1_surf.variables['OC_emiss_tra'][layer_b+imonth,:,:]=0
	num_a1_surf.variables['OC_emiss_ind'][layer_b+imonth,:,:]=0
	num_a1_surf.variables['OC_emiss_ene'][layer_b+imonth,:,:]=0
	edgar_fid.close()
num_a1_surf.close()
