...
for month in months:
	...
	"""
	## This is blocked for the different format of  REF1970 
	input_path = "/exports/csce/datastore/geos/users/s1667168/EDGAR/SHARE/NMVOC/"
	...
	for slot in range(0,5):
		...
		bio_share =bio_share + nc_fid.variables['emi_nmvoc'][:]
		nc_fid.close()
	# print 'fossil'
	for slot in range(5,18):
		...
		lat = nc_fid.variables['lat']
		lon = nc_fid.variables['lon']
		fossil_share =fossil_share + nc_fid.variables['emi_nmvoc'][:]
		nc_fid.close()
	"""	
	...
	for slot in range(3,6):
		...
		lat = nc_fid.variables['lat']
		lon = nc_fid.variables['lon']
		fossil_scenario =fossil_scenario + nc_fid.variables['emi_nmvoc'][:]
	"""	
	...
	BIGALK = (fossil*0.712+bio*0.491)*0.05*1.5/1.4/12.0*NA   # SOAG yield 5% for BIGALK  1.4 = the assumed POM/OC ratio
