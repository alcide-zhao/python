"""
This is to map sum NOx emissions from all the sectors in EDGAR 4.3.1	
emissiosn from aircraft are not included : so it is mainly NO 		
"""
...
species=['NOx'] 
emission='STAG_ENERGY' #2010 or STAG_TECH STAG_ENERGY
...
for specie in species:
	for month in months:
		...
		for sector in Sectors:
			...
			mass = mass+nc_fid.variables['emi_'+specie.lower()][:]        # kg/m2/s in EDGAR into molecules/cm2/s in CESM  1000/100/100/		
			nc_fid.close()
		...
		latitudes[:] = lat
		longitudes[:] = lon		
		CO_srfs=f.createVariable('NO_srf',np.float32,('lat','lon'))

		CO_srfs[:]=mass*NA*0.1*0.90/30  # 90% NO +10% NO2 
			
		CO_srfs.units='molecules/cm2/s'
		CO_srfs.long_name ='NO surface emission in molecules'

		f.title='NO from all emissionss'
		f.close()
