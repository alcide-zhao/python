...
def data_read_annual_mean(variable,levs):	
	...
	def mon_mean2annual_mean(scenario,time,data,levs):
		...
		for iyear in year_series:
			# # # # # # # # # # # #  DJF specifically  # # # # # # # # # # # # 
			# # # # if iyear == year_series[0] : 
				# # # # layer_b = [layer for layer in range(len(time)) if time[layer]//100 == iyear*100+1][0]
				# # # # layer_e = [layer for layer in range(len(time)) if time[layer]//100 == iyear*100+3][0]
			# # # # else:
				# # # # layer_b = [layer for layer in range(len(time)) if time[layer]//100 == (iyear-1)*100+12][0]
				# # # # layer_e = [layer for layer in range(len(time)) if time[layer]//100 == iyear*100+3][0]
			# # # # # # # # # # # #  DJF specifically  ends # # # # # # # # # # # # 
			...
			# print iyear,layer_b
			if levs:  # 4D DATA
				annual_mean=np.empty((30,30,192,288));annual_mean[:]=np.nan
	...
	def data_netcdf(scenario,FREQ,variable,levs):
		...
		days = nc_fid.variables['time'][:]; time = day2datetime(scenario,days);
		if variable =='OMEGA':
			data = -24*60*60/100*nc_fid.variables[variable][:,18,:,:] 
		else:
...
def decompose_field(field):
	if field == "stf":
		variable='PS'; lev,lat,lon,PS_T1970,PS_Edg70GO,PS_Edg70Oz,PS_EdgRef,PS_EdgEne,PS_EdgTech = data_read_annual_mean(variable,levs=False)
		...
		P_EdgEne = hybrid2p_interp(PS_EdgEne,midpoint=True,interface=False)
		P_EdgTech = hybrid2p_interp(PS_EdgTech,midpoint=True,interface=False)	
		variable='V'; lev,lat,lon,V_T1970,V_Edg70GO,V_Edg70Oz,V_EdgRef,V_EdgEne,V_EdgTech = data_read_annual_mean(variable,levs=True)
		...
		lev_new,EdgEne = streamfunction_zonal_mean(V_EdgEne,P_EdgEne,lat,lon)
		lev_new,EdgTech = streamfunction_zonal_mean(V_EdgTech,P_EdgTech,lat,lon)
		total = np.nanmean( EdgRef - T1970,axis=2)
		AAs =   np.nanmean( Edg70GO - T1970,axis=2)
		Ene =   np.nanmean( EdgRef - EdgEne,axis=2)
		Tech =  np.nanmean( EdgRef - EdgTech,axis=2)
		return lev_new,lat,total,AAs,Ene,Tech,np.nanmean(T1970,axis=2),np.nanmean(EdgRef,axis=2)
	elif field == "OMEGA":
		variable=field; lev,lat,lon,T1970,Edg70GO,Edg70Oz,EdgRef,EdgEne,EdgTech = data_read_annual_mean(variable,levs=False)
		total =  EdgRef - T1970
		AAs =    Edg70GO - T1970
		Ene =    EdgRef - EdgEne
		Tech =   EdgRef - EdgTech

		return lon,lat,total,AAs,Ene,Tech,T1970,EdgRef	
	else:
		variable=field; lev,lat,lon,T1970_f,Edg70GO_f,Edg70Oz_f,EdgRef_f,EdgEne_f,EdgTech_f = data_read_annual_mean(variable,levs=True)
		lev_new,T1970 = nterpolate2pressure(T1970_f,P_T1970,lat,lon)
		lev_new,Edg70GO = nterpolate2pressure(Edg70GO_f,P_Edg70GO,lat,lon)
		# lev_new,Edg70Oz = nterpolate2pressure(Edg70Oz_f,P_Edg70Oz,lat,lon)
		lev_new,EdgRef = nterpolate2pressure(EdgRef_f,P_EdgRef,lat,lon)
		lev_new,EdgEne = nterpolate2pressure(EdgEne_f,P_EdgEne,lat,lon)
		lev_new,EdgTech = nterpolate2pressure(EdgTech_f,P_EdgTech,lat,lon)	
		total =  EdgRef - T1970
		AAs =    Edg70GO - T1970
		Ene =    EdgRef - EdgEne
		Tech =   EdgRef - EdgTech
...
# field = 'T';lev_new,lat,T_total,T_GHG,T_AAs,T_O3,T_1970 = decompose_stf(field)
...
def plot_shade_plot(ax,x,y,shade,ContourData,colormap,colorbar_min,colorbar_max,lev_sel):
	...
	if lev_sel == "STF":
		clevs=np.array([-100,-80,-60,-40,-20,0,20,40,60,80,100])
		CS2 = plt.contour(xi,yi,ContourData,levels = clevs,linewidths=3,colors='k')
		plt.clabel(CS2,clevs,inline=False,fontsize=15,fmt= '%1.0f',inline_spacing=10) #
		ax.set_xlim([-30,30]);ax.set_xticks(np.arange(-30,31,15));ax.set_xticklabels(('30S','15S','EQ','15N','30N'));
		ax.set_ylim([200,1000]);ax.set_yticks([200,300,400,500,600,700,800,850,925,1000]);
		plt.gca().invert_yaxis()
	elif lev_sel =="OMEGA":
		clevs=np.array([0,2,4,6,8,10])*10   #-10,-8,-6,-4,-2,
		CS2 = plt.contour(xi,yi,ContourData,levels = clevs,linewidths=3,colors='k')
		plt.clabel(CS2,clevs,inline=False,fontsize=15,fmt= '%1.0f',inline_spacing=10) #
		ax.set_xlim([-180,180]);ax.set_xticks(np.arange(-180,181,15));
		ax.set_ylim([-45,45]);ax.set_yticks(np.arange(-45,46,15));ax.set_yticklabels(('30S','15S','EQ','15N','30N'));
	return CS1

def spatial_figure(ax,lons,lats,shade,ContourData,colormap,colorbar_min,colorbar_max,lev_sel,tb_lef=True,tb_bot=True): #c_bad,c_under,c_over,c_number=20,
	"""
	...
	map = Basemap(lat_0=0, lon_0=0,llcrnrlon=lon_b,llcrnrlat=lat_b,urcrnrlon=lon_e,urcrnrlat=lat_e,ax=ax,projection='cyl')
	...
	clevs=np.array([0,0.1,0.3,0.5])*100
	CS2 = map.contour(xi,yi,ContourData,levels = clevs,linewidths=3,colors='k',latlon=True)
	for level in CS2.collections:
		for kp,path in reversed(list(enumerate(level.get_paths()))):
			...
			if diameter<20: # threshold to be refined for your actual dimensions!
				del(level.get_paths()[kp])  # no remove() for Path objects:
		plt.clabel(CS2,clevs,inline=False,fontsize=15,fmt= '%1.0f',inline_spacing=10) #
	ax.set_xlim([-180,180]);ax.set_xticks(np.arange(-180,181,45));
	ax.set_xticklabels(('180W','135W','90W','45W','EQ','45E','90E','135E','180E'));
	ax.set_ylim([-30,30]);ax.set_yticks(np.arange(-30,31,10));ax.set_yticklabels(('30S','20S','10S','EQ','10N','20N','30N'));

	return colormesh

	
fig = plt.figure(facecolor='White',figsize=[30,11]);pad= 5
colormap='RdBu_r'


####STREAM FUNCITION
field = 'stf';lev_new,lat,stf_total,stf_AAs,stf_Ene,stf_Tech,stf_1970,stf_2010 = decompose_field(field)
colorbar_min=-20;colorbar_max=20;
"""
ax = plt.subplot2grid((3,6), (0, 0), colspan=2)

ax.annotate('Historical\n (2010-1970)',xy=(-0.15,0.5), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='center', va='center',rotation='vertical',fontsize=30)
ax.annotate('(a)',xy=(0.02,1.01), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='left', va='baseline',rotation='horizontal',fontsize=30)	
plot_shade_plot(ax,lat,lev_new,stf_total,stf_1970,colormap,colorbar_min,colorbar_max,lev_sel ="STF")
"""

ax = plt.subplot2grid((3,6), (0, 0), colspan=2)
ax.annotate('Mass stramfunction '+r'($\mathrm{\mathsf{\Delta}\/Tg\/s^{-1}}$)',xy=(0.5,1.05), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='center', va='baseline',rotation='horizontal',fontsize=30)
ax.annotate('BEoA\n (2010-1970)',xy=(-0.15,0.5), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='center', va='center',rotation='vertical',fontsize=30)
ax.annotate('(a)',xy=(0.02,1.01), xytext=(0, pad),
...
ax = plt.subplot2grid((3,6), (1, 0), colspan=2)
ax.annotate('Energy\n Consumption',xy=(-0.15,0.5), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='center', va='center',rotation='vertical',fontsize=30)
ax.annotate('(b)',xy=(0.02,1.01), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='left', va='baseline',rotation='horizontal',fontsize=30)	
plot_shade_plot(ax,lat,lev_new,stf_Ene,stf_2010,colormap,colorbar_min,colorbar_max,lev_sel ="STF")

ax = plt.subplot2grid((3,6), (2, 0), colspan=2)
ax.annotate('Tecchnology\n Advancements',xy=(-0.15,0.5), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='center', va='center',rotation='vertical',fontsize=30)
ax.annotate('(c)',xy=(0.02,1.01), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='left', va='baseline',rotation='horizontal',fontsize=30)	
colormesh1 = plot_shade_plot(ax,lat,lev_new,stf_Tech,stf_2010,colormap,colorbar_min,colorbar_max,lev_sel ="STF")

cbar_ax = fig.add_axes([0.07, 0.03, 0.30, 0.03])
...
del lev_new,lat,stf_total,stf_AAs,stf_Ene,stf_Tech,stf_1970,stf_2010
####OMEGA
colorbar_min=-40;colorbar_max=40;

field = 'OMEGA';lon,lat,omega_total,omega_AAs,omega_Ene,omega_Tech,omega_1970,omega_2010 = decompose_field(field)
"""
ax = plt.subplot2grid((3,6), (0, 2), colspan=4)

ax.annotate('(e)',xy=(0.02,1.01), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='left', va='baseline',rotation='horizontal',fontsize=30)	
spatial_figure(ax,lon,lat,omega_total,omega_1970,colormap,colorbar_min,colorbar_max,lev_sel ="OMEGA")
"""
ax = plt.subplot2grid((3,6), (0, 2), colspan=4)

ax.annotate('500 hPa vertical verlocity '+r'($\mathrm{\mathsf{\Delta}\/hPa\/day^{-1}}$)',xy=(0.5,1.05), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='center', va='baseline',rotation='horizontal',fontsize=30)
ax.annotate('(d)',xy=(0.02,1.01), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='left', va='baseline',rotation='horizontal',fontsize=30)	
spatial_figure(ax,lon,lat,omega_AAs,omega_1970,colormap,colorbar_min,colorbar_max,lev_sel ="OMEGA")

ax = plt.subplot2grid((3,6), (1, 2), colspan=4)

ax.annotate('(e)',xy=(0.02,1.01), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='left', va='baseline',rotation='horizontal',fontsize=30)	
spatial_figure(ax,lon,lat,omega_Ene,omega_2010,colormap,colorbar_min,colorbar_max,lev_sel ="OMEGA")

ax = plt.subplot2grid((3,6), (2, 2), colspan=4)

ax.annotate('(f)',xy=(0.02,1.01), xytext=(0, pad),
				xycoords='axes fraction', textcoords='offset points',
				ha='left', va='baseline',rotation='horizontal',fontsize=30)	
colormesh2 = spatial_figure(ax,lon,lat,omega_Tech,omega_2010,colormap,colorbar_min,colorbar_max,lev_sel ="OMEGA")

cbar_ax = fig.add_axes([0.41, 0.03, 0.56, 0.03])
char = fig.colorbar(colormesh2,orientation='horizontal',
	extend='both',cax=cbar_ax,ticks=np.round(np.arange(0,1.01,0.1)*(colorbar_max-colorbar_min)+colorbar_min,2))
	
plt.subplots_adjust(left=0.08, bottom=0.08, right=0.99, top=0.95, wspace=0.2, hspace=0.2); 
print 'saving issues'
plt.savefig('STF_OMEGA.png', format='png', dpi=300)					

"""
####VQ
field = 'VQ';lev_new,lat,VQ_AAs,VQ_Ene,VQ_Tech,VQ_2010 = decompose_stf(field)
VQ_AAs=VQ_AAs*10**(3);VQ_Ene=VQ_Ene*10**(3);VQ_Tech=VQ_Tech*10**(3);VQ_2010 =VQ_2010*10**(3)   # from kg/kg/s to g/kg m/s

colorbar_min=-1;colorbar_max=1;

ax = plt.subplot(3,2,2);
ax.annotate('Meridional Water Transport',xy=(0.5,1.03), xytext=(0, pad),
                xycoords='axes fraction', textcoords='offset points',
...
plot_shade_plot(ax,lat,lev_new,VQ_AAs,VQ_2010,colormap,colorbar_min,colorbar_max,lev_sel ="VQ")
...
plot_shade_plot(ax,lat,lev_new,VQ_Ene,VQ_2010,colormap,colorbar_min,colorbar_max,lev_sel ="VQ")
...
colormesh1 = plot_shade_plot(ax,lat,lev_new,VQ_Tech,VQ_2010,colormap,colorbar_min,colorbar_max,lev_sel ="VQ")
...
plt.subplots_adjust(left=0.06, bottom=0.10, right=0.98, top=0.96, wspace=0.1, hspace=0.15); 
plt.savefig('latitude_altitude_STF_VQ.png', format='png', dpi=1000)
...
ax.annotate('2010-1970',xy=(-0.15,0.5), xytext=(0, pad),
                xycoords='axes fraction', textcoords='offset points',
                ha='left', va='baseline',rotation='vertical',fontsize=15)
