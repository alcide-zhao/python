...
def data_readin(variable,region_key,exp):	
	...
	return Ref1970,Edg70GO,Edg70Oz,EdgRef,Edg70T10SOZ
	
def print_FaSl_GloAnuMean(variable,region_key,percent):

	def decompose_forcings(Ref970,Edg70GO,Edg70Oz,EdgRef,Edg70T10SOZ,ref,percent):
		def mean_Std(var1,var2,ref,percent):
			if percent:
				diff = 100.0*(var1 - var2)/ref
			else:
				diff = var1 - var2
			mean = round(np.nanmean(diff,axis=0),2)
			std = round(np.nanstd(diff,axis=0)/2,2)
			return mean,std
		Total,Total_S = mean_Std(EdgRef,Ref970,ref,percent)
		GHG,GHG_S = mean_Std(Edg70Oz,Edg70GO,ref,percent)
		AAs,AAs_S = mean_Std(Edg70GO,Ref970,ref,percent)
		TrO3,TrO3_S = mean_Std(EdgRef, Edg70T10SOZ,ref,percent)
		StO3,StO3_S = mean_Std(Edg70T10SOZ, Edg70Oz,ref,percent)
		
		return np.stack((Total,GHG,AAs,TrO3,StO3),axis=0)
		
	Ref970_T,Edg70GO_T,Edg70Oz_T,EdgRef_T,Edg70T10SOZ_T = data_readin(variable,region_key,exp='FullCp');    # fully coupled total response
	Ref970_F,Edg70GO_F,Edg70Oz_F,EdgRef_F,Edg70T10SOZ_F = data_readin(variable,region_key,exp='CamOnly');    # local response as the dynamics are fixed
	...
	print "========================="+variable+"============================"
	print "           TOT     GHG   AAs  TrO3  StO3"
	print 'Fast : ',decompose_forcings(Ref970_F,Edg70GO_F,Edg70Oz_F,EdgRef_F,Edg70T10SOZ_F,Ref970_T,percent)
	print 'Slow : ',decompose_forcings(Ref970_S,Edg70GO_S,Edg70Oz_S,EdgRef_S,Edg70T10SOZ_S,Ref970_T,percent)
	print 'Total: ',decompose_forcings(Ref970_T,Edg70GO_T,Edg70Oz_T,EdgRef_T,Edg70T10SOZ_T,Ref970_T,percent)
