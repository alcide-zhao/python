...
def data_readin(variable,FREQ):	
	...
	lon,lat,EdgRef70A = data_netcdf('EdgRef70AP',FREQ,variable)
	_,_,EdgRef = data_netcdf('EdgRefP',FREQ,variable)
	_,_,EdgEne = data_netcdf('EdgEneP',FREQ,variable)
	_,_,EdgTech = data_netcdf('EdgTechP',FREQ,variable)
	return lon,lat,EdgRef70A,EdgRef,EdgEne,EdgTech
...
def print_domain_mean(variable,FREQ):
	...
	def diff_sig(vairable1,variable2):
		...
		return dif,sig
	if variable == "precip":	
		lon,lat,EdgRef70A_C,EdgRef_C,EdgEne_C,EdgTech_C = data_readin('PRECL',FREQ);
		lon,lat,EdgRef70A_L,EdgRef_L,EdgEne_L,EdgTech_L = data_readin('PRECC',FREQ);
		EdgRef70A=(EdgRef70A_C+EdgRef70A_L)
		EdgRef=(EdgRef_C+EdgRef_L)
		EdgEne=(EdgEne_C+EdgEne_L)
		EdgTech=(EdgTech_C+EdgTech_L)		
	elif variable == "CF":	  # cloud forcing
		lon,lat,EdgRef70A_S,EdgRef_S,EdgEne_S,EdgTech_S = data_readin('SWCF',FREQ);
		lon,lat,EdgRef70A_L,EdgRef_L,EdgEne_L,EdgTech_L = data_readin('LWCF',FREQ);
		EdgRef70A=(EdgRef70A_S+EdgRef70A_L)
		...
		lon,lat,EdgRef70A_S,EdgRef_S,EdgEne_S,EdgTech_S = data_readin('FSNT',FREQ);
		lon,lat,EdgRef70A_L,EdgRef_L,EdgEne_L,EdgTech_L = data_readin('FLNT',FREQ);
		EdgRef70A=(EdgRef70A_S-EdgRef70A_L)
		EdgRef=(EdgRef_S-EdgRef_L)
		EdgEne=(EdgEne_S-EdgEne_L)
		EdgTech=(EdgTech_S-EdgTech_L)	
	else:
		lon,lat,EdgRef70A,EdgRef,EdgEne,EdgTech = data_readin(variable,FREQ);
		
	AEROSOL,AEROSOL_s = diff_sig(EdgRef,EdgRef70A)
	...
	for region_key in region_keys:
		...
		print "=========="+region_key+"=========="
		print 'AEROSOL  ',dif_std_for_region(EdgRef,EdgRef70A,mask)
		print 'ENE      ',dif_std_for_region(EdgRef,EdgEne,mask)
		print 'TECH     ',dif_std_for_region(EdgRef,EdgTech,mask)
...
def zonal_mean_unceer(variable,mask):
	def data_read_zonal_mean(variable,mask):
		...
		def data_netcdf(scenario,variable):
			...
			elif variable =="VQ" or variable == "VT":
				data = np.nanmean(nc_fid.variables[variable][:,23:30,:,:],axis=1)  # 850hpa
			elif variable =="PRECC" or variable == "PRECL":
		...
		lat,EdgRef70A = data_netcdf('EdgRef70AP',variable)
		_,EdgRef = data_netcdf('EdgRefP',variable)
		_,EdgEne = data_netcdf('EdgEneP',variable)
		_,EdgTech = data_netcdf('EdgTechP',variable)
		return lat,EdgRef70A,EdgRef,EdgEne,EdgTech

	if variable == "precip":	
		lat,EdgRef70A_C,EdgRef_C,EdgEne_C,EdgTech_C = data_read_zonal_mean('PRECL',mask);
		lat,EdgRef70A_L,EdgRef_L,EdgEne_L,EdgTech_L = data_read_zonal_mean('PRECC',mask);
		EdgRef70A=(EdgRef70A_C+EdgRef70A_L)
		EdgRef=(EdgRef_C+EdgRef_L)
		EdgEne=(EdgEne_C+EdgEne_L)
		EdgTech=(EdgTech_C+EdgTech_L)
	elif variable == "CF":	  # cloud forcing
		lat,EdgRef70A_S,EdgRef_S,EdgEne_S,EdgTech_S = data_read_zonal_mean('SWCF',mask);
		lat,EdgRef70A_L,EdgRef_L,EdgEne_L,EdgTech_L = data_read_zonal_mean('LWCF',mask);
		EdgRef70A=(EdgRef70A_S+EdgRef70A_L)
		EdgRef=(EdgRef_S+EdgRef_L)
		EdgEne=(EdgEne_S+EdgEne_L)
		EdgTech=(EdgTech_S+EdgTech_L)
	elif variable == "NSLTOA":	  # cloud forcing
		lat,EdgRef70A_S,EdgRef_S,EdgEne_S,EdgTech_S = data_read_zonal_mean('FSNT',mask);
		lat,EdgRef70A_L,EdgRef_L,EdgEne_L,EdgTech_L = data_read_zonal_mean('FLNT',mask);
		EdgRef70A=(EdgRef70A_S-EdgRef70A_L)
		EdgRef=(EdgRef_S-EdgRef_L)
		EdgEne=(EdgEne_S-EdgEne_L)
		EdgTech=(EdgTech_S-EdgTech_L)			
		...
		# print variable,mask
		lat,EdgRef70A,EdgRef,EdgEne,EdgTech = data_read_zonal_mean(variable,mask); #,EdgEne,EdgTech
	elif variable == "HT":  # heat transport
		lat,EdgRef70A_l,EdgRef_L,EdgEne_L,EdgTech_L = data_read_zonal_mean('FLNT',mask='All');
		lat,EdgRef70A_S,EdgRef_S,EdgEne_S,EdgTech_S = data_read_zonal_mean("FSNT",mask='All');
		lat,EdgRef70A_LHFLX,EdgRef_LHFLX,EdgEne_LHFLX,EdgTech_LHFLX = data_read_zonal_mean('LHFLX',mask='All');
		lat,EdgRef70A_SHFLX,EdgRef_SHFLX,EdgEne_SHFLX,EdgTech_SHFLX = data_read_zonal_mean("SHFLX",mask='All');
		lat,EdgRef70A_FLNS,EdgRef_FLNS,EdgEne_FLNS,EdgTech_FLNS = data_read_zonal_mean('FLNS',mask='All');
		lat,EdgRef70A_FSNS,EdgRef_FSNS,EdgEne_FSNS,EdgTech_FSNS = data_read_zonal_mean("FSNS",mask='All');
		lat,EdgRef70A_PRECSC,EdgRef_PRECSC,EdgEne_PRECSC,EdgTech_PRECSC = data_read_zonal_mean('PRECSC',mask='All');
		lat,EdgRef70A_PRECSL,EdgRef_PRECSL,EdgEne_PRECSL,EdgTech_PRECSL = data_read_zonal_mean("PRECSL",mask='All');		
		lat,EdgRef70A_QFLX,EdgRef_QFLX,EdgEne_QFLX,EdgTech_QFLX = data_read_zonal_mean('QFLX',mask='All');
		lat,EdgRef70A_PRECC,EdgRef_PRECC,EdgEne_PRECC,EdgTech_PRECC = data_read_zonal_mean("PRECC",mask='All');
		lat,EdgRef70A_PRECL,EdgRef_PRECL,EdgEne_PRECL,EdgTech_PRECL = data_read_zonal_mean("PRECL",mask='All');
		if mask=='All':
			EdgRef70A =  inferred_heat_transport(EdgRef70A_S-EdgRef70A_L,lat)
			...
			EdgRef70A =  inferred_heat_transport(-(EdgRef70A_FLNS-EdgRef70A_FSNS +EdgRef70A_LHFLX+EdgRef70A_SHFLX +(EdgRef70A_PRECSC+EdgRef70A_PRECSL)/2),lat)
			...
			EdgRef70A =  inferred_heat_transport(EdgRef70A_S-EdgRef70A_L+(EdgRef70A_FLNS-EdgRef70A_FSNS +EdgRef70A_LHFLX+EdgRef70A_SHFLX +(EdgRef70A_PRECSC+EdgRef70A_PRECSL)/2),lat)
	...
	mean_AAs,P25_AAs,P75_AAs = 	decompose_mean_std(EdgRef,EdgRef70A)
	mean_Ene,P25_Ene,P75_Ene = 	decompose_mean_std(EdgRef,EdgEne)
	mean_Tec,P25_Tec,P75_Tec = decompose_mean_std(EdgRef,EdgTech)
	
	AAs  = np.concatenate((mean_AAs,P25_AAs,P75_AAs),axis=0)
	Ene  = np.concatenate((mean_Ene,P25_Ene,P75_Ene),axis=0)
	Tech = np.concatenate((mean_Tec,P25_Tec,P75_Tec),axis=0)
	return lat, AAs, Ene,Tech

def plot_zonal_mean_uncertainty(ax,x,y1,y2,y3):
	...
	ax.plot(x,y1[0:192],'-',color="b",linewidth=3,label = 'Best Estmation')
...
index ='TS'		  #  TS  FSNT FLNS CDNUMC ACTREL TGCLDLWP SWCF LWCF LHFLX precip
zonal_mean=True
...
if zonal_mean:	
	...
	lat, AAs, Ene,Tech= zonal_mean_unceer(index,mask = 'All');
	ax = plt.subplot(2,2,4);
	ax.annotate('(d) Zonal Mean',xy=(0.02,1.01), xytext=(0, pad),
					xycoords='axes fraction', textcoords='offset points',
					ha='left', va='baseline',rotation='horizontal',fontsize=20)	
	ax1 = plot_zonal_mean_uncertainty(ax,lat,AAs, Ene,Tech)
